...
class ViewController: NSViewController {
    ...
    private func setupViews() {
        ...
        enableTokenMenu.state = NSControl.StateValue.on
        enableSearchIcon.state = NSControl.StateValue.on
        setCustomLeftView.state = NSControl.StateValue.off
        enableTokenMenu.state = NSControl.StateValue.on
    }
    ...
    @IBAction func enableClearButton(_ sender: NSButton) {
        tokenFieldRandom.shouldDisplayClearButton = sender.state == NSControl.StateValue.on
    }

    @IBAction func enableSearchIcon(_ sender: NSButton) {
        if sender.state == NSControl.StateValue.on {
            setCustomLeftView.state = NSControl.StateValue.on
            tokenFieldRandom.leftView = nil
        }
        tokenFieldRandom.shouldDisplaySearchIcon = sender.state == NSControl.StateValue.on
    }
    
    @IBAction func setLeftView(_ sender: NSButton) {
        if sender.state == NSControl.StateValue.on {
            tokenFieldRandom.leftView = imageView
            enableSearchIcon.state = NSControl.StateValue.off
        } else {
            tokenFieldRandom.leftView = nil
        }
    }
    ...
    @IBAction func enableTokenMenu(_ sender: NSButton) {
        tokenFieldRandom.shouldEnableTokenMenu = sender.state == NSControl.StateValue.on
